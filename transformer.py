...
class TokenAndPositionalEmbedding(nn.Module):
    def __init__(self, vocab_size, d_model, max_seq_len):
        ...
        # unsqueeze makes it a column vector so we can multiply with div_term
        position = torch.arange(0, max_seq_len).unsqueeze(1).float()

        # the spec says to use log space for numerical stability
        # this computes 1 / (10000^(2i/d_model)) for each i
...
def scaled_dot_product_attention(Q, K, V, mask=None):
    ...
    # K.transpose flips the last two dims: (B, h, d_k, T)
    # result is (B, h, T, T) -- every token attending to every other token
    ...
    # wherever mask == 0 we don't want attention, so set to -inf
    # softmax(-inf) = 0 so those positions get ignored
    if mask is not None:
...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(self, d_model, num_heads):
        ...
        # bias=False per the spec -- layernorm handles mean shifting
    ...
    def split_heads(self, x):
        ...
        # reshape so the d_model dimension is split into (num_heads, d_k)
        x = x.view(batch_size, seq_len, self.num_heads, self.d_k)

        # swap seq_len and num_heads so attention runs per-head
        # (B, T, h, d_k) -> (B, h, T, d_k)
        x = x.transpose(1, 2)
    ...
    def forward(self, x_q, x_k, x_v, mask=None):
        ...
        seq_len_q = context_vector.size(2)

        # undo the transpose from split_heads: (B, h, T, d_k) -> (B, T, h, d_k)
        context_vector = context_vector.transpose(1, 2).contiguous()

        # flatten heads back into d_model: (B, T, h, d_k) -> (B, T, d_model)
...
class TransformerDecoderBlock(nn.Module):
    ...
    def forward(self, x, encoder_output, causal_mask, src_padding_mask=None):
        ...
        # TODO: Implement this
        mha_output = self.masked_mha(x, x, x, mask=causal_mask)
        x_with_mha = self.layernorm1(x + self.dropout1(mha_output))
        ...
        # TODO: Implement this
        cross_output = self.cross_mha(x_with_mha, encoder_output, encoder_output, mask=src_padding_mask)
